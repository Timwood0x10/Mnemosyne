...
async fn war_profile() {
    ...
    let text: &str = &full[..full.char_indices().nth(50_000).map(|(i,_)| i).unwrap_or(full.len())];
    println!("========== Compiler Profile ==========\n");
    println!("Sample: {} chars\n", text.len());

    // Phase 1: Document
    let t0 = Instant::now();
    let doc = Document::from_text("War and Peace", "novel", text);
    let t1 = t0.elapsed();
    ...
    println!("Phase                        time        %        ");
    println!("──────────────────────────────────────────────────");
    println!("1. Document load       {:>8.3}s  {:>5.1}%", t1.as_secs_f64(), t1.as_secs_f64()/total.as_secs_f64()*100.0);
    println!("2. Registry build      {:>8.3}s  {:>5.1}%", t2.as_secs_f64(), t2.as_secs_f64()/total.as_secs_f64()*100.0);
    println!("3. Profile extract     {:>8.3}s  {:>5.1}%", t3.as_secs_f64(), t3.as_secs_f64()/total.as_secs_f64()*100.0);
    println!("4. Entity register     {:>8.3}s  {:>5.1}%", t4.as_secs_f64(), t4.as_secs_f64()/total.as_secs_f64()*100.0);
    println!("5. Chunk plan          {:>8.3}s  {:>5.1}%", t5.as_secs_f64(), t5.as_secs_f64()/total.as_secs_f64()*100.0);
    println!("6. Sentence split      {:>8.3}s  {:>5.1}%", t6.as_secs_f64(), t6.as_secs_f64()/total.as_secs_f64()*100.0);
    println!("7. Story Compiler      {:>8.3}s  {:>5.1}%", t7.as_secs_f64(), t7.as_secs_f64()/total.as_secs_f64()*100.0);
    println!("──────────────────────────────────────────────────");
    println!("Total                  {:>8.3}s", total.as_secs_f64());

    println!("\nStats: {} entities, {} sentences, {} events", ctx.entities.len(), sent_texts.len(), ctx.events.len());
}
