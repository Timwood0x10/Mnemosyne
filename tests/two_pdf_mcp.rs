...
//! corpus/2.pdf file, then verifies the graph rows were created.
//!
...
async fn build_server() -> (
    ...
    let fact_store =
        Arc::new(lore_scope::fact_store::SqliteFactStore::open_in_memory().expect("fact store"));
...
/// Objective: End-to-end attach + materialize of the real 2.pdf.pdf.
...
#[tokio::test]
async fn attach_and_materialize_two_pdf() {
    let pdf_path = std::path::Path::new("corpus/2.pdf");
    if !pdf_path.exists() {
        eprintln!("⚠  corpus/2.pdf not present — skipping e2e PDF test");
        return;
    }
    ...
    let attach_resp = call_tool(
        ...
        serde_json::json!({
            ...
            "source_name": "two-pdf",
            "entity_links": [
                {"external_name": "Quasimodo", "canonical_name": "卡西莫多", "source": "two-pdf"}
            ]
        }),
    )
    .await;
    ...
    assert_eq!(payload["source_name"], "two-pdf");
    ...
    let ingest_resp = call_tool(
        &server,
        "knowledge_ingest",
        serde_json::json!({"source_name": "two-pdf", "mode": "materialize"}),
    )
    .await;
    ...
    // `load_document` uses the file stem as title: corpus/2.pdf → "2".
    let doc = store
        .find_document_by_title("2")
        .await
        .expect("query doc")
        .expect("materialized document must exist in the graph");
    assert_eq!(doc.title, "2");
