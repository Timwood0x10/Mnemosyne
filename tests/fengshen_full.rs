...
async fn fengshen_full() {
    ...
    for (i, line) in text.lines().enumerate() {
        ...
        if !chapter_ids.contains_key(&ch) {
            ...
            // Create chapters table entry (needed by search_evidence JOIN)
            // Use INSERT OR IGNORE to handle duplicate chapter_no across doc boundaries
            let _ = conn.execute(
                "INSERT OR IGNORE INTO chapters (id, doc_id, chapter_no, title, content, start_offset, end_offset, created_at)
                 VALUES (?1, ?2, ?3, '', '', 0, 0, 0)",
