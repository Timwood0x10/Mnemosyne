...
### 关键任务
- [ ] P1. 定义 `PersonaSignalExtractor` trait + 原型句库加载（JSON 可配置）。
- [ ] P2. 实现 `EmbeddingPersonaExtractor`：原型向量缓存 + 余弦相似度 + argmax 阈值判定。
- [ ] P3. 实现 `KeywordPersonaExtractor`（平移现有关键词逻辑，作为回退）。
- [ ] P4. 接入说话人归属：按发言者把信号写到对应 entity。
- [ ] P5. 阈值校准 + 混合语料回归（复用 `tests/` 中小说/对话用例）。
- [ ] P6. 实现记忆调和决策器：候选事实 vs 已存事实 → ADD / NOOP / 冲突标记（v3 累积）。
- [ ] P7. 调和器单元测试：重复识别、正反转变标记、时间上下文保留。
...
- [ ] A1. 定义 `persona_facts` 读取接口：按 entity 拉取全部 `agent_personality` 标记的事实（含说话人归属）。
- [ ] A2. 实现 `persona_check` MCP 工具 handler：
...
- [ ] A3. 注册工具 + 输入 schema + 单元测试（含正反标记冲突用例）。
- [ ] A4. 接入 host 契约文档：在生成前调用，冲突时提示 host 改稿或降级。

### 阶段 B：人设注入（`persona_inject`）—— 陪伴型入口

- [ ] B1. 设计**人设卡数据结构**（结构化稳定约束，替代散落关键词）：
  - `identity`（我是谁）、`persona`（性格/口吻）、`style`（言说风格）、`taboos`（禁忌/绝对不说的）、`relationship`（与用户的关系状态）。
- [ ] B2. 人设卡来源：a) 导入 JSON 人设卡文件；b) 从 `agent_personality` 事实自动聚合生成。
- [ ] B3. 实现 `persona_inject` 工具：返回可拼进 system prompt 的文本/JSON 人设包。
- [ ] B4. 支持按 `tenant_id` 多角色多份人设卡切换（适配小说多角色 + 真实对话多 agent）。

### 阶段 C：打开陪伴信号 + 关系状态

- [ ] C1. 将 `COMPANION_EXTRACT_GRAYSCALE` 置为 `false`，接入 `agent_fact_compile` 编译链路。
- [ ] C2. 用混合语料（小说 + 真实对话）回归验证 `emotion_series` / `self_cognition` / `repeated_themes` 的提取质量。
- [ ] C3. 设计**关系状态表**：`relationship_state`（亲密度、关系阶段、最近共同话题、情感趋势）。
- [ ] C4. 实现 `relationship_update`：每轮对话后根据情绪信号增量更新关系状态。
- [ ] C5. 实现 `relationship_query`：跨会话读取当前关系/情感状态快照。
- [ ] C6. **实现"人设演进时间线"**：把累积的 `fact`（含时间上下文）按时间组织成"一个人的完整变化过程"，复用现有 `timeline` / `key_events` / `story_events` 能力，像解析三国演义某人生平一样，可输出「起点 → 关键转变点 → 现状」的完整轨迹。

### 阶段 D：记忆衰减/遗忘管理

- [ ] D1. 设计衰减策略配置（`decay` 参数：按时间/按重要性/按访问频率）。
- [ ] D2. 实现后台衰减任务：对低重要度 / 久未访问的 persona 记忆降权或归档。
- [ ] D3. 提供 `memory_decay` 触发入口 + 手动可调，防止长期陪伴记忆膨胀。
- [ ] D4. **衰减不破坏演进证据**：降权/归档只影响检索排序，不删除历史 `fact`，确保"人设演进时间线"仍可完整重建。

### 阶段 E：工程化收尾

- [ ] E1. 更新 `README` / `README.zh.md` 工具清单与 host 集成契约。
- [ ] E2. 补充端到端测试：注入 → 生成 → 校验 → 回写 → 状态更新全链路。
- [ ] E3. 代码评审 + 性能/存储回归。
...
- [ ] `PersonaSignalExtractor` 在 embedding 可用时用语义匹配，不可用时自动回退关键词，行为一致。
- [ ] 语义引擎能正确识别正反标记冲突（如"我不喜欢" vs 草稿"我喜欢"）并给出证据。
- [ ] 语义引擎在**小说 + 真实对话**混合语料上都能按说话人归属到正确 entity。
- [ ] `persona_inject` 能返回完整人设包，支持多角色切换。
- [ ] `companion_extract` 灰度打开后，情绪/自我认知/重复主题可落库并可查询。
- [ ] `relationship_state` 能跨会话反映亲密度与情感趋势变化。
- [ ] 记忆调和器能识别重复（NOOP）、标记正反转变点，且不覆盖已有人设事实（v3 累积）。
- [ ] **人设演进时间线**能输出某人的完整变化过程（起点 → 关键转变点 → 现状），在小说与真实对话语料上都成立。
- [ ] 记忆衰减任务可配置并可手动触发，不丢高价值人设事实，且不破坏演进时间线。
