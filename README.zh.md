...
词表在 `config/` 里，**词是你的，动作名是引擎的**。把自定义写进
`config/markers_zh.user.json`（`config/` 下任何 `*.user.json` 都会被读取）：
...
1. **桶名**必须是引擎已定义的动作：`喜欢` / `like` / `love` / `prefer` / `dislike` /
   `hate` / `准备` / `plan` / `want` / `feel` / `belief` / `stuck` / `life_event`。
   写错会在自检里报错并列出合法值（以前会静默变成"事件"事实）；
2. **`.user.json` 里的词追加**到随包词表上，多个文件按文件名顺序合并；
3. **`_remove` 用来删词**——随包词表里在你场景下误报的词，只能这样关掉；
   删除没命中也会被报告出来（通常是拼错了）。

随包的 `markers_zh.json` / `markers_en.json` 只是参考样例：**升级会覆盖它们**，
所以别把自定义写进去。改完重启即可，无需重新编译：`.user.json` 不存在时照样能跑
（用随包表）；两个随包文件都读不到时会退回一份很小的内置兜底表。

词表从**资源根目录**解析：默认取当前目录或可执行文件所在目录，也可以用
`MNEMOSYNE_HOME` 显式指定。
...
它会打印资源根目录、加载了哪些词表文件、每个动作最终多少词、被删/被忽略/重复的条目，
以及运行时会读取的其它 `config/` 文件（缺哪个一眼能看到）。词表有问题时退出码非 0，
可以直接当部署前的门禁。
